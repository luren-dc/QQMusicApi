    ...
    def login(
        ...
        if login_type == "get_qrcode":
            ...
            if "二维码未失效" in response.text:
                ...
                refresh_url = re.findall(r"'(https:.*?)'", response.text)[0]
                return {
                    "code": 200,
                    "data": {"state": state, "uin": uin, "refresh_url": refresh_url},
                }
            return {"code": 200, "data": {"state": state}}
        elif login_type == "get_cookie":
            if not refresh_url:
                raise ParamsException("缺少必要参数")
            response = requests.get(refresh_url, allow_redirects=False, verify=False)
